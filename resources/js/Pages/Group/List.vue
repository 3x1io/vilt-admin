<template>
    <app-layout title=" {{ $title }}">
        <div class="container grid px-6 mx-auto">
            <Header :canCreate="$attrs.canCreate" title="Group" button="Create Group"
                @buttonAction="create">
            </Header>
        ...
        <br>
        <br>
        <CreateModal :url="url" title="Group" :show="createModal" :edit="edit" :errors="errors" :item="form"
            :rows="rows" @close="createModal = !createModal" @success="success" />
        <ViewModal title="View Group" :show="viewModal" :item="form" :rows="rows"
            @close="viewModal = !viewModal" />
        <DeleteModal :url="url" title="Delete Group" :show="deleteModal" :item="form"
            @close="deleteModal = !deleteModal" @success="success" />
        <BulkModal :url="url" title="Bulk Action On Group" :action="bulkActionTitle" :bulk="bulk"
