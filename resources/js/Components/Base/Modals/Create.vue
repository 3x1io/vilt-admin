<template>
    <JetDialogModal :show="showModal" @end="close">
        ...
        <template #content>
            <form class="grid grid-cols-1 gap-3 lg:grid-cols-1" action="" v-if="!edit">
                <slot name="create-top"></slot>
                    <div v-for="(field, key) in rowFilterCreate" :key="key" class="">
                        ...
                        <div v-else-if="(field.type === 'schema' || field.type === 'schema-area') && field.create">
                            <div>
                                <div class="">
                                    <JetLabel
                                        :for="field.field"
                                        :value="field.label"
                                    />
                                    <div class="px-4 py-4 mt-2 border rounded-default">
                                        <div
            ...
            <form class="grid grid-cols-1 gap-3 md:grid-cols-1" action="" v-else>
                ...
                <div v-for="(field, key) in rowFilterEdit" :key="key" class="">
                    ...
                    <div v-else-if="field.type === 'switch' && field.edit" class="px-6">
                        <div>
                            <ViltSwitch
                                :label="field.label"
                                :name="field.field"
                    ...
                    <div v-else-if="(field.type === 'schema' || field.type === 'schema-area') && field.edit">
                        <div>
                            <div class="px-4">
                                <JetLabel
                                    :for="field.field"
                                    :value="field.label"
                                />
                                <div class="px-4 pb-3 mt-2 border rounded-default">
                                    <div
                                        v-for="(input, index) in field.options"
                                        :key="index"
                                        class="mt-2"
    ...
    computed: {
        ...
        rowFilterCreate() {
            ...
            for (let i = 0; i < rows.length; i++) {
                if (
                    this.arraysEqual(rows[i].reactive, this.form[rows[i].row]) && rows[i].create
                ) {
        ...
        rowFilterEdit() {
            ...
            for (let i = 0; i < rows.length; i++) {
                if (
                    this.arraysEqual(rows[i].reactive, this.form[rows[i].row]) && rows[i].edit
                ) {
                    rowValue.push(rows[i]);
